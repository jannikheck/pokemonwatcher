...
# Welche Karten? Primaer ueber die 16 Set-IDs der EX-Aera (Ruby & Sapphire bis
# Power Keepers) - das trifft exakt die gewuenschten Sets.
SET_ID_QUERY = " OR ".join(f"set.id:ex{i}" for i in range(1, 17))

# Ersatzabfrage ueber die Serie, falls die IDs sich mal aendern sollten.
SERIES_QUERY = "set.series:ex"
...
def _fetch_page(query, page, headers):
    ...
    # Die API liefert zeitweise 500/502. Das sind kurze Aussetzer, deshalb lieber
    # oefter und kuerzer warten statt lange - spart im Zweifel Minuten Laufzeit.
    for versuch in range(1, 6):
        ...
        if r.status_code == 429 or r.status_code >= 500:
            wartezeit = 4 * versuch     # 4, 8, 12, 16, 20 Sekunden
    ...
    return None, "Nach 5 Versuchen keine Antwort (Serverproblem bei pokemontcg.io)"
...
def fetch_cards():
    ...
    # Reihenfolge nach Praxiserfahrung: die Set-IDs haben sich als zuverlaessig
    # erwiesen (1722 Karten) und treffen exakt die 16 gewuenschten Sets. Die
    # Serien-Abfrage bleibt als Ersatz, falls sich IDs mal aendern.
    abfragen = [
        (SET_ID_QUERY, "Set-IDs ex1-ex16"),
        (SERIES_QUERY, "Serie 'ex'"),
    ]
    ...
    for query, beschreibung in abfragen:
        ...
        while True:
            ...
            if fehler:
                print(f"  [FEHLER] Seite {page}: {fehler}")
                if cards:
                    # Wichtig: was schon geladen ist, wird BEHALTEN. Fehlende Karten
                    # bekommen diesmal einfach kein Signal - das ist harmlos, weil
                    # jeder Posten einzeln im State gefuehrt wird.
                    print(f"  -> Behalte die {len(cards)} bereits geladenen Karten.")
                break
